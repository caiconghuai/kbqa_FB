...
# load data
train_loader = SeqRankingLoader(args.train_file, args.neg_size, len(rel_vocab), args.gpu)
print('load train data, batch_num: %d\tbatch_size: %d'
      %(train_loader.batch_num, train_loader.batch_size))
valid_loader = SeqRankingLoader(args.valid_file, args.neg_size, len(rel_vocab), args.gpu)
...
print(header)
ones = torch.autograd.Variable(torch.ones(train_loader.batch_size * args.neg_size))
if args.cuda:
    ones = ones.cuda()
...
for epoch in range(1, args.epochs+1):
    ...
    for batch_idx, batch in enumerate(train_loader.next_batch()):
        ...
        n_correct += torch.gt(pos_score, neg_score).cpu().data.sum()
        n_total += train_loader.batch_size*args.neg_size
        ...
        if iterations % args.dev_every == 0:
            ...
            for valid_batch_idx, valid_batch in enumerate(valid_loader.next_batch()):
                ...
                n_dev_correct += torch.gt(val_pos_score, val_neg_score).cpu().data.sum()
...
#            P, R, F = evaluation(gold_list, pred_list)
            ...
            dev_acc = 100. * n_dev_correct / (valid_loader.batch_num*valid_loader.batch_size*args.neg_size)
